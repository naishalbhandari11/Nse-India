...
let displayFromDate = ''; // Client-side filter: only show signals >= this date
let displayToDate = '';   // Client-side filter: only show signals <= this date
...
async function performScanFast() {
    ...
    // Store display filters (used for client-side filtering of results)
    ...
    // Store current parameters — fromDate used for scan filtering, not passed to detail page
    ...
    // Create cache key — includes fromDate so different date ranges are cached separately
    ...
    try {
        // PERFORMANCE MODE: dedicated /api/performance-scan endpoint
        // from_date/to_date = company activity filter; full history analyzed for those companies
        const fromParam = fromDate ? `&from_date=${fromDate}` : '';
        const url = `/api/performance-scan?target=${target}&stop_loss=${stopLoss}&holding_days=${holdingDays}${fromParam}&to_date=${toDate}&indicator=${indicator}`;
...
function renderUngroupedResults(results, tbody, startIndex) {
    results.forEach((result, index) => {
        ...
        // Use scan data directly — enrichment is removed
        ...
        const soldOutSignals  = result.sold_out_signals || 0;
        const openTrades      = result.open_trades      || 0;
        const notTradedSignals= result.not_traded_signals || 0;
        ...
        row.innerHTML = `
            ...
            <td class="center">
                <a href="/performance-detail/${encodeURIComponent(result.symbol)}?indicator=${encodeURIComponent(indicatorParam)}&target=${currentScanParams.target}&stop_loss=${currentScanParams.stopLoss}&days=${currentScanParams.holdingDays}&to_date=${currentScanParams.toDate || ''}" 
...
function renderGroupedResults(groups, tbody, startIndex) {
    groups.forEach((group, index) => {
        ...
        mainRow.innerHTML = `
            ...
            <td class="center">
                <a href="/performance-detail/${encodeURIComponent(group.symbol)}?indicator=${encodeURIComponent(matchedIndicatorsParam)}&target=${currentScanParams.target}&stop_loss=${currentScanParams.stopLoss}&days=${currentScanParams.holdingDays}&to_date=${currentScanParams.toDate || ''}" 
        ...
        const indicatorsTable = group.indicators.map(ind => {
            ...
            return `
                <tr>
                    ...
                    <td class="center">
                        <a href="/performance-detail/${encodeURIComponent(group.symbol)}?indicator=${encodeURIComponent(ind.indicator)}&target=${currentScanParams.target}&stop_loss=${currentScanParams.stopLoss}&days=${currentScanParams.holdingDays}&to_date=${currentScanParams.toDate || ''}" 
    ...
    sorted.sort((a, b) => {
        ...
        switch (sortBy) {
            ...
            case 'success':
                // Success rate = profit / completed (exclude OPEN and NOT_TRADED)
                {
                    const aP = a.profit_signals    || 0;
                    const aL = a.loss_signals      || 0;
                    const aS = a.sold_out_signals  || 0;
                    const aC = aP + aL + aS;
                    valA = aC > 0 ? aP / aC : 0;

                    const bP = b.profit_signals    || 0;
                    const bL = b.loss_signals      || 0;
                    const bS = b.sold_out_signals  || 0;
                    const bC = bP + bL + bS;
                    valB = bC > 0 ? bP / bC : 0;
                }
                break;
            case 'netProfit':
                valA = typeof a.total_net_profit !== 'undefined' ? a.total_net_profit : (a.net_profit_loss || 0);
                valB = typeof b.total_net_profit !== 'undefined' ? b.total_net_profit : (b.net_profit_loss || 0);
                break;
            case 'symbol':
...
async function enrichUngroupedStats(rows) {
    // Enrichment removed — data from /api/performance-scan is already complete and correct.
    // Previously this called /api/scanner/analyze per row causing thousands of requests.
}
